...
def one_hot_encoding(train, variables):
    ...
    for var in tqdm(variables):
        one_hot_dict[var] = []
        train[var] = train[var].fillna(train[var].min())
        ...
        if len(values)>2:
            for val in values:
                ...
                one_hot_cols.append(f'{var}_{int(val)}')
                one_hot_dict[var].append(val)
            train.drop([var], axis=1, inplace=True)
        else:
            one_hot_cols.append(var)

    return train, one_hot_cols, one_hot_dict
...
def preprocess(fold, df_temp, high_missing_cols, binary_cols, ordinal_cols, normal_cols, cat_cols):
    ...
    df_arr = np.reshape(df_num[normal_cols + ordinal_cols].fillna(0).values, (int(df_num.shape[0] / 13), 13, len(normal_cols + ordinal_cols)))
    print('3D input:', df_arr.shape)
    np.save(f'neural_network/data/train_num_{fold}.npy', df_arr)
    ...
    np.save(f'neural_network/data/train_high_{fold}.npy', train_num_agg.values)
    ...
    np.save(f'neural_network/data/train_skew_{fold}.npy', train_num_agg.values)
    ...
    target.to_pickle(f'neural_network/data/targets_{fold}.pkl')
...
train, outliers = outlier_fixer(train, normal_cols+high_missing_cols+ordinal_cols)

joblib.dump(outliers, 'neural_network/data/outliers.joblib')
...
target  = pd.read_csv(DATA_DIR2+ 'train_labels.csv')
target ['customer_ID'] = encoder.transform(target ['customer_ID'])
...
fold=0
for train_index, test_index in skf.split(target.index, target['target']):
    target.loc[test_index, 'fold']=fold
    fold+=1

train, one_hot_cols, one_hot_dict = one_hot_encoding(train, cat_cols)
train = train.replace([np.inf, -np.inf], np.nan)

del cat_cols

joblib.dump(one_hot_cols, 'neural_network/data/one_hot_cols.joblib')
joblib.dump(one_hot_dict, 'neural_network/data/one_hot_dict.joblib')

train = train.merge(target, on=['customer_ID'], how='left')

train.loc[train['customer_ID'] == train['customer_ID'].min()].to_csv('check.csv', index=False)

for fold in range(Nfolds):
    df_temp = train.loc[train['customer_ID'].isin(target.loc[target['fold'] == fold, 'customer_ID'].to_list())].reset_index(drop=True)
    msg = preprocess(fold, df_temp, high_missing_cols, binary_cols, ordinal_cols, normal_cols, one_hot_cols)
